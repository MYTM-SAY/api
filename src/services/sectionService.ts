...
const getSectionsByClassroomId = async (classroomId: number, userId: number) => {
  ...
  const isJoined = await isHasRole(userId, classroomExist.communityId,
    [Role.MODERATOR, Role.OWNER, Role.MEMBER]);

  if (!isJoined)
    throw new APIError("You must be joined", 403)
...
const createSection = async (data: z.infer<typeof CreateSectionSchema>, userId: number) => {
...
const deleteSection = async (id: number, userId: number) => {
  ...
  const isOwnerOrMod = await isHasRole(userId, sectionExists.Classroom.communityId,
    [Role.MODERATOR, Role.OWNER]);
