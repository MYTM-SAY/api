...
async function createUserProfile(profiledata: any, userId: number) {
  ...
  return UserProfileRepo.createProfile(validatedData, userId)
}

async function updateUserProfile(profiledata: any, userId: number) {
  if (!userId || isNaN(userId)) throw new APIError('Invalid User ID', 400)
  ...
  return UserProfileRepo.updateProfile(validatedData, userId)
}
